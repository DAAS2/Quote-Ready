...
const SOURCE_META = {
  ai_analysis: { icon: Bot, label: "AI analysis", hint: "Gemini extracted facts from new input" },
  voice_update: { icon: Mic, label: "Voice update", hint: "A field note changed the scope" },
  manual_edit: { icon: UserRound, label: "Manual update", hint: "Operator edited the job" },
  fallback: { icon: Bot, label: "Fallback analysis", hint: "Live AI unavailable — precomputed result" },
  seed: { icon: Bot, label: "Initial analysis", hint: "Baseline scope from the enquiry" },
} as const;
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs leading-relaxed text-muted-foreground">
        Every re-analysis or field update creates a new scope version. Open a version to see
        exactly what changed and how the recommendation moved.
      </p>
      ...
      <ol className="space-y-2.5">
        {[...ordered].reverse().map((pack, idx) => {
          ...
          const isLatest = pack.version === latest.version;
          const changeCount =
            (diff?.field_changes.length ?? 0) +
            (diff?.new_risk_flags.length ?? 0) +
          ...
          return (
            ...
            >
              <details className="group">
                <summary className="flex cursor-pointer list-none flex-wrap items-center gap-x-2 gap-y-1 px-3.5 py-3 transition-colors hover:bg-accent/40 [&::-webkit-details-marker]:hidden">
                  ...
                  <ChevronDown
                    className="size-3.5 text-muted-foreground/50 transition-transform duration-200 group-open:rotate-180"
                    aria-hidden
                  />
                </summary>
                ...
                <div className="border-t px-3.5 py-3">
                  <p className="text-xs text-muted-foreground">
                    {meta.hint} — {pack.recommended_action.title.toLowerCase()}.
                  </p>
                  ...
                  {(diff && changeCount > 0) ? (
                    ...
                    <p className="mt-2 text-xs text-muted-foreground">
                      No fact changes — this version re-ran the same scope (e.g. a re-analysis).
                    </p>
                  )}

                  {pack.override_reasons.length > 0 && (
                    <p className="mt-2.5 text-[11px] leading-relaxed text-muted-foreground/80">
                      Why: {pack.override_reasons.join(" ")}
                    </p>
                  )}
                </div>
              </details>
